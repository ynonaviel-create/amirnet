/* ================= אמירנט — שכבת האסטרטגיה =================
   ...
   וגם לא צריך. חיפשתי במאגר של 58 מבחני עבר — 464 השלמת משפטים, 231
...
const FINDINGS = [
  ...
  { part: 'הבנת הנקרא', kind: 'works', val: '61%', n: 572,
    ...
    why: '351 מתוך 572 השאלות נוקבות בפסקה במפורש ("in the second paragraph"). ' +
         'הפסקה שמצוינת מכילה 82% מהראיות לתשובה, והיא 29% מאורך הקטע. ' +
         'כלומר: אל תקרא את הקטע. קרא את השאלה, לך לפסקה, קרא שש שורות.' },
...
function paintFocus() {
  ...
  if (done) {
    ...
    w.innerHTML = '<h4>' + (R.pick === it.answer ? 'נכון' : 'התשובה: (' + (it.answer + 1) + ')') + '</h4>' +
      '<p class="note">' + (R.full
        ? 'פתחת את הקטע המלא. במבחן זה עולה כדקה. נסה בפעם הבאה להחליט מהפסקה בלבד — ' +
          'ב-82% מהמקרים כל הראיות שם.'
        : 'ענית מהפסקה בלבד, בלי לקרוא ' + Math.round(100 - 100 * sc.text.length / p.text.length) +
          '% מהקטע. זה בדיוק הקצב שהמבחן דורש.') + '</p>';
    mid.appendChild(w);
  }
...
function view(v) {
  ...
  tile(dr, 'דריל פסילה — ניסוח מחדש', 'כאן שני כללים נמדדים חותכים אפשרות עוד לפני הקריאה.',
    () => startElim('rs', 6));
  tile(dr, 'קריאה ממוקדת', 'שאלה, פסקה אחת, תשובה. בלי לקרוא את הקטע.',
    startFocus, '29% מהטקסט');
  v.appendChild(dr);
  ...
  f.appendChild(el('p', 'note',
    'כל שורה כאן נבדקה על מאגר של 58 מבחני עבר עם התשובות הרשמיות. ' +
